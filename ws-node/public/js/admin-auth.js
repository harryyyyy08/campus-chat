...
const API_BASE = `${_protocol}//${_host}/campus-chat/api/index.php`;
let adminToken = null;
...
async function initAdminApp(user) {
  ...
  if (_av)
    _av.textContent = (user.full_name || user.username).charAt(0).toUpperCase();

  const badge = document.getElementById("adminRoleBadge");
  if (myRole === "super_admin") {
    badge.textContent = "Super Admin";
    badge.className = "admin-role-badge super";
    document.getElementById("adminRoleIcon").innerHTML =
...
function initAdminSocket(token) {
  const _host = window.location.hostname || "localhost";
  const _protocol = window.location.protocol === "https:" ? "https:" : "http:";
  const WS_URL = `${_protocol}//${_host}:3001`;
