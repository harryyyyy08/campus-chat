...
if ($method === "POST" && $path === "/conversations/direct") {
  ...
  if ($other_username === "")
    json_response(["error" => "other_username required"], 400);
  ...
  if ($other_id === $me_id)
    json_response(["error" => "Cannot create conversation with yourself"], 400);
  ...
  $stmt->execute([$me_id, $other_id]);
  $existing = $stmt->fetch();
  if ($existing)
    json_response(["conversation_id" => (int) $existing["conversation_id"], "created" => false, "other_user" => ["id" => $other_id, "username" => $other["username"], "full_name" => $other["full_name"]]]);
...
if ($method === "POST" && $path === "/conversations/group") {
  ...
  if (count($member_ids) < 1)
    json_response(["error" => "Add at least 1 other member"], 400);
  ...
  if ((int) $stmt->fetchColumn() !== count($member_ids))
    json_response(["error" => "One or more users not found"], 404);
  $pdo->beginTransaction();
  try {
...
if ($method === "POST" && $path === "/conversations/members") {
  ...
  $new_uid = (int) ($in["user_id"] ?? 0);
  if (!$cid || !$new_uid)
    json_response(["error" => "conversation_id and user_id required"], 400);
  $stmt = $pdo->prepare("SELECT type FROM conversations WHERE id = ?");
  $stmt->execute([$cid]);
  $conv = $stmt->fetch();
  if (!$conv || $conv["type"] !== "group")
    json_response(["error" => "Not a group conversation"], 400);
  $stmt = $pdo->prepare("SELECT role FROM conversation_members WHERE conversation_id = ? AND user_id = ?");
  $stmt->execute([$cid, $me_id]);
  ...
  $stmt->execute([$cid, $new_uid]);
  if ($stmt->fetchColumn())
    json_response(["error" => "User is already a member"], 409);
...
if ($method === "DELETE" && $path === "/conversations/members") {
  ...
  $me_id = (int) $claims["sub"];
  $cid = (int) ($in["conversation_id"] ?? 0);
  $rem_uid = (int) ($in["user_id"] ?? 0);
  if (!$cid || !$rem_uid)
    json_response(["error" => "conversation_id and user_id required"], 400);
  $stmt = $pdo->prepare("SELECT role FROM conversation_members WHERE conversation_id = ? AND user_id = ?");
  $stmt->execute([$cid, $me_id]);
  $my_role = $stmt->fetchColumn();
  if ($my_role !== "admin" && $rem_uid !== $me_id)
    json_response(["error" => "Only admins can remove members"], 403);
...
if ($method === "POST" && $path === "/conversations/leave") {
  ...
  if ($my_role === "admin") {
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM conversation_members WHERE conversation_id = ? AND role = 'admin'");
    $stmt->execute([$cid]);
    if ((int) $stmt->fetchColumn() <= 1) {
      $stmt = $pdo->prepare("SELECT COUNT(*) FROM conversation_members WHERE conversation_id = ? AND user_id <> ?");
      $stmt->execute([$cid, $me_id]);
      if ((int) $stmt->fetchColumn() > 0)
        json_response(["error" => "Promote another admin before leaving"], 400);
    }
  }
...
if ($method === "GET" && $path === "/conversations/unread") {
  ...
  foreach ($stmt->fetchAll() as $r)
    $counts[(int) $r["conversation_id"]] = (int) $r["unread_count"];
...
if ($method === "GET" && $path === "/conversations") {
  ...
  foreach ($convs as &$c) {
    ...
    if ($last) {
      $last["id"] = (int) $last["id"];
      $last["sender_id"] = (int) $last["sender_id"];
      $last["has_attachment"] = !empty($last["attachment_id"]);
    }
...
if ($method === "POST" && $path === "/conversations/request") {
  ...
  $other_username = trim((string) ($in["other_username"] ?? ""));
  if ($other_username === "" || $body === "")
    json_response(["error" => "other_username and body required"], 400);
  ...
  $other_id = (int) $other["id"];
  if ($other_id === $me_id)
    json_response(["error" => "Cannot send request to yourself"], 400);

  // Faculty/admin skip the request system
  if (
    in_array($me["role"], ["faculty", "admin", "super_admin"]) ||
  ...
  if ($existing)
    json_response(["error" => "Already chatting — use direct message.", "conversation_id" => (int) $existing], 409);
  ...
  if ($req && $req["status"] === "pending")
    json_response(["error" => "You already have a pending request to this user"], 409);
...
if ($method === "GET" && $path === "/conversations/is-request") {
  ...
  $cid = (int) ($_GET["id"] ?? 0);
  if (!$cid)
    json_response(["is_request" => false]);
