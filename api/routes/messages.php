...
if ($method === "POST" && $path === "/messages") {
  ...
  foreach ($attachment_ids as $att_id) {
    ...
    if (!$stmt->fetchColumn())
      json_response(["error" => "Invalid attachment: $att_id"], 400);
  }
...
if ($method === "PATCH" && preg_match('#^/messages/(\d+)$#', $path, $m)) {
  ...
  if ($new_body === "")
    json_response(["error" => "body required"], 400);
  ...
  if ((int) $msg["sender_id"] !== $user_id)
    json_response(["error" => "Cannot edit another user's message"], 403);
  if ($msg["is_deleted"])
    json_response(["error" => "Cannot edit a deleted message"], 400);
  if ((time() - strtotime($msg["created_at"])) > 15 * 60)
    json_response(["error" => "Edit window has expired (15 minutes)"], 403);
  if (!$msg["body"] && $msg["attachment_id"])
    json_response(["error" => "Cannot edit attachment-only messages"], 400);
...
if ($method === "DELETE" && preg_match('#^/messages/(\d+)$#', $path, $m)) {
  ...
  if ((int) $msg["sender_id"] !== $user_id)
    json_response(["error" => "Cannot delete another user's message"], 403);
  if ($msg["is_deleted"])
    json_response(["error" => "Message already deleted"], 400);
  if ((time() - strtotime($msg["created_at"])) > 15 * 60)
    json_response(["error" => "Delete window has expired (15 minutes)"], 403);
...
if ($method === "POST" && preg_match('#^/messages/(\d+)/react$#', $path, $m)) {
  ...
  if ($emoji === "")
    json_response(["error" => "emoji required"], 400);
