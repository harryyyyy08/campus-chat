...
if ($method === "GET" && $path === "/admin/reset-requests") {
  ...
  $stmt = $pdo->prepare("SELECT r.id, r.user_id, r.requested_at, r.status, r.reset_method, u.username, u.full_name, d.name AS department, u.role AS user_role FROM password_reset_requests r JOIN users u ON u.id = r.user_id LEFT JOIN departments d ON d.id = u.department WHERE r.status = 'pending' AND r.reset_method = 'admin' ORDER BY r.requested_at ASC");
  $stmt->execute();
  $rows = $stmt->fetchAll();
  foreach ($rows as &$r) {
    $r["id"] = (int) $r["id"];
...
if ($method === "POST" && preg_match('#^/admin/reset-requests/(\d+)/approve$#', $path, $m)) {
  ...
  if ($req["status"] !== "pending")
    json_response(["error" => "Request already resolved"], 409);
...
if ($method === "GET" && $path === "/admin/users") {
  ...
  if (!is_admin($my_role))
    json_response(["error" => "Admin access required"], 403);
  $stmt = $pdo->prepare("SELECT u.id, u.username, u.full_name, u.status, u.role, d.name AS department, u.created_at FROM users u LEFT JOIN departments d ON d.id = u.department ORDER BY u.status ASC, u.created_at DESC");
  $stmt->execute();
  $users = $stmt->fetchAll();
  foreach ($users as &$u)
    $u["id"] = (int) $u["id"];
...
if ($method === "POST" && $path === "/admin/users/disable") {
  ...
  if (!$user_id)
    json_response(["error" => "user_id required"], 400);
  if ($user_id === (int) $claims["sub"])
    json_response(["error" => "Cannot disable your own account"], 400);
  $stmt = $pdo->prepare("SELECT role FROM users WHERE id = ?");
  $stmt->execute([$user_id]);
  $target_role = $stmt->fetchColumn();
  if (is_super_admin($target_role) && !is_super_admin($my_role))
    json_response(["error" => "Cannot disable a super admin"], 403);
...
if ($method === "POST" && $path === "/admin/users/role") {
  ...
  if (!is_super_admin($my_role))
    json_response(["error" => "Only super admins can change roles"], 403);
  $user_id = (int) ($in["user_id"] ?? 0);
  $role = $in["role"] ?? "";
  if (!$user_id)
    json_response(["error" => "user_id required"], 400);
  if (!in_array($role, ["student", "faculty", "admin", "super_admin"]))
...
if ($method === "GET" && $path === "/admin/storage") {
  ...
  $stmt = $pdo->prepare("SELECT COUNT(*) as total_files, COALESCE(SUM(file_size),0) AS total_bytes FROM attachments");
  $stmt->execute();
  $stats = $stmt->fetch();
...
if ($method === "POST" && $path === "/admin/cleanup") {
  ...
  $freed = 0;
  if (empty($ids)) {
    $stmt = $pdo->prepare("SELECT id, stored_name, file_size FROM attachments WHERE last_accessed < DATE_SUB(NOW(), INTERVAL 6 MONTH)");
    $stmt->execute();
    $rows = $stmt->fetchAll();
  } else {
    $ph = implode(',', array_fill(0, count($ids), '?'));
    $stmt = $pdo->prepare("SELECT id, stored_name, file_size FROM attachments WHERE id IN ({$ph})");
    $stmt->execute($ids);
    $rows = $stmt->fetchAll();
  }
  foreach ($rows as $row) {
    $file_path = UPLOAD_DIR . $row["stored_name"];
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM attachments WHERE stored_name = ? AND id <> ?");
    $stmt->execute([$row["stored_name"], $row["id"]]);
    if ((int) $stmt->fetchColumn() === 0 && file_exists($file_path))
      unlink($file_path);
...
if ($method === "GET" && $path === "/admin/conversations") {
  ...
  $params = [];
  if ($type_filter && in_array($type_filter, ["direct", "group"])) {
    $sql .= " AND c.type = ?";
    $params[] = $type_filter;
  }
  if ($search !== "") {
    $sql .= " AND (c.name LIKE ? OR c.id IN (SELECT DISTINCT conversation_id FROM messages WHERE body LIKE ?))";
    $params[] = "%$search%";
    $params[] = "%$search%";
  }
...
if ($method === "GET" && $path === "/admin/messages/search") {
  ...
  $q = trim($_GET["q"] ?? "");
  if (strlen($q) < 2)
    json_response(["messages" => [], "note" => "Query too short"], 200);
  $like = "%" . $q . "%";
...
if ($method === "POST" && $path === "/admin/messages/flag") {
  ...
  if (!$message_id)
    json_response(["error" => "message_id required"], 400);
