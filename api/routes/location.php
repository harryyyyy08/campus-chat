...
if ($method === "POST" && $path === "/admin/location-zones") {
  ...
  if (!$name || !$building)
    json_response(["error" => "name and building are required"], 400);
  if (!valid_cidr($cidr))
    json_response(["error" => "Invalid CIDR format (e.g. 10.0.1.0/24)"], 400);
...
if ($method === "PUT" && preg_match('#^/admin/location-zones/(\d+)$#', $path, $m)) {
  ...
  $zone = $existing->fetch();
  if (!$zone)
    json_response(["error" => "Zone not found"], 404);
  ...
  if (!$name || !$building)
    json_response(["error" => "name and building are required"], 400);
  if (!valid_cidr($cidr))
    json_response(["error" => "Invalid CIDR format"], 400);
